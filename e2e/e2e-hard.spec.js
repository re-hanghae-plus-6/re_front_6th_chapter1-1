...
test.describe('E2E: 쇼핑몰 전체 사용자 시나리오', () => {
  ...
  test.describe('5. 장바구니 완전한 워크플로우', () => {
    test('여러 상품 추가, 수량 조절, 선택 삭제 전체 시나리오', async ({
      ...
      // 두 상품이 모두 있는지 확인
      await expect(page.locator('#cart-modal-container')).toContainText(
        'PVC 투명 젤리 쇼핑백',
      );
      await expect(page.locator('#cart-modal-container')).toContainText(
        '샷시 풍지판',
      ...
      // 총 금액 업데이트 확인
      await expect(page.locator('#cart-modal-container')).toMatchAriaSnapshot(`
    - text: /총 금액 670원/
    - button "전체 비우기"
    - button "구매하기"
    `);
      ...
      await expect(page.locator('#cart-modal-container')).not.toContainText(
        'PVC 투명 젤리 쇼핑백',
      );
      await expect(page.locator('#cart-modal-container')).toContainText(
        '샷시 풍지판',
