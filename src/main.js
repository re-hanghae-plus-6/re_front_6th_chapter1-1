...
function loadCart() {
  try {
    state.cart = JSON.parse(localStorage.getItem("shopping_cart") || "{}");
  } catch {
    state.cart = {};
  }
}
...
function showToast(message = "장바구니에 추가되었습니다.") {
...
function updateCartBadge() {
  ...
  const old = cartIcon.querySelector("span");
  if (old) old.remove();
...
function addToCart(productId, quantity = 1) {
  ...
  if (state.cart[productId]) {
    ...
    const product = state.products.find((p) => String(p.productId) === String(productId));
...
function formatPrice(won) {
  return `${Number(won).toLocaleString()}원`;
}
  ...
  function renderCartModal() {
      ...
      <div class="flex flex-col max-h-[calc(90vh-120px)]">
            ...
            ${cartItems
              ...
              <div class="flex items-center py-3 border-b border-gray-100 cart-item" data-product-id="${product.productId}">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-sm text-gray-600 mt-1">${formatPrice(product.lprice)}</p>
                ...
                <div class="text-right ml-3">
                  <p class="text-sm font-medium text-gray-900 price-field" data-product-id="${product.productId}">${formatPrice(product.lprice * quantity)}</p>
        ...
        <div class="sticky bottom-0 bg-white border-t border-gray-200 p-4 space-y-2">
          <div class="flex justify-between items-center mb-1 text-sm">
            ...
            <span class="font-bold" id="cart-modal-total-amount">${formatPrice(totalPrice)}</span>
          </div>
    ...
    modal.querySelectorAll(".quantity-increase-btn").forEach((btn) => {
      btn.onclick = () => {
        ...
        qtyInput.value = state.cart[pid].quantity;
        priceField.textContent = formatPrice(state.cart[pid].quantity * state.cart[pid].product.lprice);
        modal.querySelector("#cart-modal-total-amount").textContent = formatPrice(
          Object.values(state.cart).reduce((s, c) => s + c.product.lprice * c.quantity, 0),
        );
      };
    });
    ...
    removeSelBtn.onclick = () => {
      ...
      selectedPids.forEach((pid) => delete state.cart[pid]);
      saveCart();
      updateCartBadge();
      renderCartModal();
    };
