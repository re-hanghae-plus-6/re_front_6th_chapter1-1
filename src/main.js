...
// 컴포넌트라고 생각하셈
function main() {
  const state = createReactiveState(1);
  const view = {
    currentView: null,
    render(props) {
      if (!this.currentView) {
        this.currentView = testTemplate(props);
    ...
    update(props) {
      const newView = testTemplate(props);
      document.body.replaceChild(newView, this.currentView);
  ...
  state.onChange((value) => {
    view.render({
      child: simpleParagraph({ text: "심플 텍스트 변경됨" }),
      title: `타이틀 ${value}`,
      onClickButton: (e) => {
        e.preventDefault();
        state.updateState((prev) => prev + 1);
      },
    });
  });

  view.render({
    child: simpleParagraph({ text: "심플 텍스트 " }),
    title: "타이틀",
    onClickButton: () => {
      state.updateState((prev) => prev + 1);
    },
...
const testTemplate = ({ child, title, onClickButton }) => {
  const a = document.createElement("div");
  a.innerText = title;

  const button = document.createElement("button");
  button.innerText = "업데이트 버튼";
  button.addEventListener("click", onClickButton);
  a.appendChild(button);

  console.log(child);

  if (child) a.appendChild(child);
