export default function renderProductDetail(state) {
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2">
              <button id="cart-icon-btn" class="relative p-2 text-gray-700 hover:text-gray-900 transition-colors">
                ...
                ${state.cart.length > 0 ? `<span class="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">${state.cart.length}</span>` : ""}
              </button>
            </div>
          </div>
        </div>
          ...
          <div class="p-4">
            ...
            <div>
              ...
              <div class="text-sm text-gray-700 leading-relaxed mb-6">
                ${product.description || `${product.title}에 대한 상세 설명입니다. ${product.brand} 브랜드의 우수한 품질을 자랑하는 상품으로, 고객 만족도가 높은 제품입니다.`}
              </div>
            </div>
          </div>
          <!-- 수량 선택 및 액션 -->
