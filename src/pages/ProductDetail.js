...
function render() {
  //   즉시 실행함수로 만들어서 기존 return은 유지했음
  document.body.querySelector("#root").innerHTML = (function () {
    if (!state.product) {
      return /* HTML */ `
      ...
      <main class="max-w-md mx-auto px-4 py-4">
        ...
        <nav class="mb-4">
          <div class="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <button class="breadcrumb-link" data-category1="생활/건강">${state.product.category1}</button>
            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
            <button class="breadcrumb-link" data-category2="생활용품">${state.product.category2}</button>
          </div>
        </nav>
        <!-- 상품 상세 정보 -->
          ...
          <div class="p-4">
            ...
            <div>
              ...
              <div class="text-sm text-gray-600 mb-4">
                <div class="text-sm text-gray-600 mb-4">재고 ${state.product.stock || 107}개</div>
              </div>
              <!-- 설명 -->
          ...
          <div class="p-4">
            <div class="grid grid-cols-2 gap-3 responsive-grid" id="related-products-list">
              ${state.relatedProducts
                .filter(
                  (p) =>
                    p.productId !== state.product.id &&
                    (p.category2 === state.product.category2 || p.category1 === state.product.category1),
                )
                .map(
                  (related) => `
                <div class="bg-gray-50 rounded-lg p-3 related-product-card cursor-pointer" data-product-id="${related.productId}">
                  <div class="aspect-square bg-white rounded-md overflow-hidden mb-2">
                    <img src="${related.image}" alt="${related.title}" class="w-full h-full object-cover" loading="lazy">
                  </div>
                  <h3 class="text-sm font-medium text-gray-900 mb-1 line-clamp-2">${related.title}</h3>
                  <p class="text-sm font-bold text-blue-600">${related.lprice}원</p>
                </div>
              `,
                )
                .join("")}
