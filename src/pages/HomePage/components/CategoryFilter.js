export default function CategoryFilter({ state }) {
  ...
  return /*html */ `
    <div class="space-y-2">
        ...
        ${
          filters?.category1 &&
          /*html */ `&gt; <button data-category1="${filters.category1}" class="text-xs hover:text-blue-800 hover:underline">
           ${filters.category1}
            </button>`
        }
        ...
        ${
          filters?.category2 &&
          /*html */ `&gt; <button data-category2="${filters.category2}" class="text-xs hover:text-blue-800 hover:underline">
          ${filters.category2}
            </button>`
        }
        </div>
        ${state.loading ? '<div class="text-sm text-gray-500">카테고리 로딩 중...</div>' : ""}
        <!-- 1depth 카테고리 필터 -->
        ${
          !filters?.category1 && !state.loading
