export const ProductDetailPage = ({ product = null }) => /*html*/ `
      ...
      <main class="max-w-md mx-auto px-4 py-4">
        ...
        <nav class="mb-4">
          <div class="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <button class="breadcrumb-link" data-category1="${product?.category1 || "생활/건강"}">
              ${product?.category1 || "생활/건강"}
            </button>
            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
            <button class="breadcrumb-link" data-category2="${product?.category2 || "생활용품"}">
              ${product?.category2 || "생활용품"}
            </button>
          </div>
        </nav>
        <!-- 상품 상세 정보 -->
        <div class="bg-white rounded-lg shadow-sm mb-6">
          ...
          <div class="p-4">
            <div class="aspect-square bg-gray-100 rounded-lg overflow-hidden mb-4">
              <img src="${product?.image || "https://shopping-phinf.pstatic.net/main_8506721/85067212996.1.jpg"}" alt="${product?.title || "PVC 투명 젤리 쇼핑백 1호 와인 답례품 구디백 비닐 손잡이 미니 간식 선물포장"}" class="w-full h-full object-cover product-detail-image">
            </div>
            <!-- 상품 정보 -->
            <div>
              <p class="text-sm text-gray-600 mb-1">${product?.brand || product?.mallName || ""}</p>
              <h1 class="text-xl font-bold text-gray-900 mb-3">${product?.title || "PVC 투명 젤리 쇼핑백 1호 와인 답례품 구디백 비닐 손잡이 미니 간식 선물포장"}</h1>
              <!-- 평점 및 리뷰 -->
              <div class="flex items-center mb-3">
                ...
                <span class="ml-2 text-sm text-gray-600">${product?.rating || 4}.0 (${(product?.reviewCount || 749).toLocaleString()}개 리뷰)</span>
              </div>
              <!-- 가격 -->
              <div class="mb-4">
                <span class="text-2xl font-bold text-blue-600">${parseInt(product?.lprice || 220).toLocaleString()}원</span>
              </div>
              <!-- 재고 -->
              <div class="text-sm text-gray-600 mb-4">
                재고 ${(product?.stock || 107).toLocaleString()}개
              </div>
              <!-- 설명 -->
              <div class="text-sm text-gray-700 leading-relaxed mb-6">
                ${product?.description || `${product?.title || "PVC 투명 젤리 쇼핑백 1호 와인 답례품 구디백 비닐 손잡이 미니 간식 선물포장"}에 대한 상세 설명입니다. 브랜드의 우수한 품질을 자랑하는 상품으로, 고객 만족도가 높은 제품입니다.`}
              </div>
            </div>
          </div>
          <!-- 수량 선택 및 액션 -->
          <div class="border-t border-gray-200 p-4">
            ...
            <!-- 액션 버튼 -->
            <button id="add-to-cart-btn" data-product-id="${product?.productId || "85067212996"}" class="w-full bg-blue-600 text-white py-3 px-4 rounded-md 
