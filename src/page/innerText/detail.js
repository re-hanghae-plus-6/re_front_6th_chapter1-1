export default function detail(detailState, state) {
  const 상세페이지_로딩 = `
      ...
      ${
        ...
        <nav class="mb-4">
          <div class="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <button class="breadcrumb-link" data-category1=${detailState.product.category1}>
                ${detailState.product.category1}
            </button>
            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
            <button class="breadcrumb-link" data-category2=${detailState.product.category2}>
                ${detailState.product.category2}
            </button>
          </div>
        </nav>
        <!-- 상품 상세 정보 -->
        <div class="bg-white rounded-lg shadow-sm mb-6">
          ...
          <div class="p-4">
            <div class="aspect-square bg-gray-100 rounded-lg overflow-hidden mb-4">
              <img src=${detailState.product.image} alt=${
                detailState.product.title
              } class="w-full h-full object-cover product-detail-image">
            </div>
            <!-- 상품 정보 -->
            <div>
              <p class="text-sm text-gray-600 mb-1"></p>
              <h1 class="text-xl font-bold text-gray-900 mb-3">${detailState.product.title}</h1>
              <!-- 평점 및 리뷰 -->
              <div class="flex items-center mb-3">
                <div class="flex items-center">
                ${(() => {
                  // 평점(별점) 소수점 첫째자리까지, 최대 5
                  const rating = Number(detailState.product.rating) || 0;
                  ...
                  // 5개 이하면 남은 별 개수 표시
                  if (rating < 5) {
                    starsHtml += `<span class="ml-1 text-xs text-gray-500">(${5 - rating}개 부족)</span>`;
                  }
                  return starsHtml;
                })()}
                  
                </div>
                <span class="ml-2 text-sm text-gray-600">${detailState.product.rating}.0 (${detailState.product.reviewCount}개 리뷰)</span>
              </div>
              <!-- 가격 -->
              <div class="mb-4">
                <span class="text-2xl font-bold text-blue-600">${detailState.product.lprice}원</span>
              </div>
              <!-- 재고 -->
              <div class="text-sm text-gray-600 mb-4">
                재고 ${detailState.product.stock}개
              </div>
              <!-- 설명 -->
          ...
          <div class="border-t border-gray-200 p-4">
            <div class="flex items-center justify-between mb-4">
              ...
              <div class="flex items-center">
                ...
                <input type="number" id="quantity-input" value=${detailState.count} min="1" max="107" class="w-16 h-8 text-center text-sm border-t border-b border-gray-300
