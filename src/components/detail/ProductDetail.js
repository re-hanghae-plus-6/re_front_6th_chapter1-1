export default function ProductDetail({ productInfo }) {
  ...
  <nav class="mb-4">
    <div class="flex items-center space-x-2 text-sm text-gray-600">
      ...
      <button class="breadcrumb-link" data-category1="${productInfo.category1}">
      ${productInfo.category1}
      </button>
      <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
      </svg>
      <button class="breadcrumb-link" data-category2="${productInfo.category2}">
  ...
  <div class="bg-white rounded-lg shadow-sm mb-6">
    ...
    <div class="p-4">
      <div class="aspect-square bg-gray-100 rounded-lg overflow-hidden mb-4">
        <img src="${productInfo.image}" alt="${productInfo.title}" class="w-full h-full object-cover product-detail-image">
      </div>
      <!-- 상품 정보 -->
      <div>
        <p class="text-sm text-gray-600 mb-1"></p>
        <h1 class="text-xl font-bold text-gray-900 mb-3">${productInfo.title}</h1>
        <!-- 평점 및 리뷰 -->
        <div class="flex items-center mb-3">
          <div class="flex items-center">
            ${renderStars(productInfo.rating)}
          </div>
          <span class="ml-2 text-sm text-gray-600">${productInfo.rating} (${productInfo.reviewCount}개 리뷰)</span>
        </div>
        <!-- 가격 -->
        <div class="mb-4">
          <span class="text-2xl font-bold text-blue-600">${productInfo.lprice}원</span>
        </div>
        <!-- 재고 -->
    ...
    <div class="border-t border-gray-200 p-4">
      ...
      <!-- 액션 버튼 -->
      <button id="add-to-cart-btn" data-product-id="${productInfo.productId}" 
