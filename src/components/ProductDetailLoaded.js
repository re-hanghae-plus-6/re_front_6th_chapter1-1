...
import { ProductDetailHeader } from "./Header.js";

export const RelatedProduct = ({ product }) => {
  const { title, lprice, image, productId } = product;
  return /* HTML */ `
...
const ProductDetailLoadedContent = ({ product, relatedProducts }) => {
  const { title, lprice, image, category1, category2, productId, reviewCount, stock, description } = product;

  console.log("ProductDetailLoadedContent - category1:", category1, "category2:", category2);
   ...
   <main class="max-w-md mx-auto px-4 py-4">
          ...
          <nav class="mb-4">
            <div class="flex items-center space-x-2 text-sm text-gray-600">
              ...
              <button class="breadcrumb-link" data-breadcrumb="category1" data-category1="${category1 || ""}">
                ${category1 || "카테고리"}
              </button>
              ${
                category2
              ...
              <button class="breadcrumb-link" data-breadcrumb="category2" data-category1="${category1 || ""}" data-category2="${category2 || ""}">
                ${category2}
              </button>
              `
                  : ""
            ...
            <div class="p-4">
              ...
              <div>
                ...
                <div class="flex items-center mb-3">
                  ...
                  <span class="ml-2 text-sm text-gray-600">4.0 (${reviewCount}개 리뷰)</span>
                </div>
                <!-- 가격 -->
                <div class="mb-4">
...
const ProductDetailLoaded = async ({ productId }, state) => {
  if (!productId) {
    return /* HTML */ `
  ...
  try {
    ...
    const response = await getProducts({ category2: product.category2 });
    const relatedProducts = response.products.filter((relatedProduct) => relatedProduct.productId !== productId);

    return /* HTML */ `
      <div class="min-h-screen bg-gray-50">
        ${ProductDetailHeader({ cart: state.cartItems })}
        ${ProductDetailLoadedContent({ product, relatedProducts, state })}
    ...
    return /* HTML */ `
      <div class="min-h-screen bg-gray-50 flex items-center justify-center">
        <div class="text-center">
          <h1 class="text-2xl font-bold text-gray-800 mb-4">상품을 찾을 수 없습니다</h1>
          <p class="text-gray-600 mb-4">요청하신 상품 정보를 불러올 수 없습니다.</p>
          <button onclick="window.history.back()" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
            이전 페이지로 돌아가기
          </button>
        </div>
