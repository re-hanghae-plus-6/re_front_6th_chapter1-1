export const CartModal = ({ cart }) => {
  console.log("CartModal called", cart, cart.length);

  return `
    <div id="cart-modal-bg" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          ...
          cart.length === 0
            ...
            : `<div class="flex flex-col max-h-[calc(90vh-120px)]">
                ...
                <div class="flex-1 overflow-y-auto">
                  ...
                  ${[...new Map(cart.map((item) => [item.productId, item])).values()]
                    .map(
                      (item) => `
                      <div class="flex items-center py-3 border-b border-gray-100 cart-item" data-product-id="${item.productId}">
                        ...
                        <div class="flex-1 min-w-0">
                          ...
                          <div class="flex items-center mt-2">
                            ...
                            <input type="number" value="${
                              cart.filter((c) => c.productId === item.productId).length
                            }"min="1" class="quantity-input w-12 h-7 text-center text-sm border-t border-b border-gray-300 focus:ring-1 focus:ring-blue-500 focus:border-blue-500" disabled data-product-id="${item.productId}">
                        ...
                        <div class="text-right ml-3">
                          <p class="text-sm font-medium text-gray-900">
                            ${item.lprice * cart.filter((c) => c.productId === item.productId).length}원
                          </p>
              ...
              <div class="sticky bottom-0 bg-white border-t border-gray-200 p-4">
                <div class="flex justify-between items-center mb-4">
                  ...
                  <span class="text-xl font-bold text-blue-600">${cart.reduce((acc, cur) => acc + cur.lprice * cur.quantity, 0)}원</span>
                </div>
                <div class="space-y-2">
