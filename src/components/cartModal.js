  ...
  return `
    <div class="flex items-center py-3 border-b border-gray-100 cart-item" data-product-id="${item.productId}">
      ...
      <div class="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden mr-3 flex-shrink-0">
        <img src="${product.image}" alt="${product.title}" class="w-full h-full object-cover cursor-pointer cart-item-image" data-product-id="${item.productId}">
      </div>
      <!-- 상품 정보 -->
      ...
      <div class="text-right ml-3">
        <p class="text-sm font-medium text-gray-900">
          ${(product.lprice * item.quantity).toLocaleString()}원
        </p>
    ...
    <div class="relative bg-white rounded-t-lg sm:rounded-lg shadow-xl w-full max-w-md sm:max-w-lg max-h-[90vh] overflow-hidden">
      ...
      <div class="sticky top-0 bg-white border-b border-gray-200 p-4 flex items-center justify-between">
        <h2 class="text-lg font-bold text-gray-900 flex items-center">
          ...
          장바구니
          <span class="text-sm font-normal text-gray-600 ml-1">(${computed.totalCount})</span>
        </h2>
        <button id="cart-modal-close-btn" class="text-gray-400 hover:text-gray-600 p-1">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
      ...
      <div class="sticky bottom-0 bg-white border-t border-gray-200 p-4">
        ...
        <div class="flex justify-between items-center mb-4">
          ...
          <span class="text-xl font-bold text-blue-600">${(computed.totalPrice || 0).toLocaleString()}원</span>
        </div>
        <!-- 액션 버튼들 -->
        <div class="space-y-2">
  ...
  return `
    <div class="relative bg-white rounded-t-lg sm:rounded-lg shadow-xl w-full max-w-md sm:max-w-lg max-h-[90vh] overflow-hidden">
      ...
      <div class="flex flex-col max-h-[calc(90vh-120px)]">
        ...
        <div class="p-4 border-b border-gray-200 bg-gray-50">
          <label class="flex items-center text-sm text-gray-700">
            <input type="checkbox" id="cart-modal-select-all-checkbox" class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 mr-2" ${computed.selectedItems.length === computed.totalCount ? "checked" : ""}>
            전체선택 (${computed.totalCount}개)
          </label>
        </div>
        <!-- 아이템 목록 -->
      ...
      <div class="sticky bottom-0 bg-white border-t border-gray-200 p-4">
        ...
        <div class="flex justify-between items-center mb-2 text-sm text-gray-600">
          <span>선택된 상품 ${computed.selectedItems.length}개</span>
          <span>${(computed.selectedPrice || 0).toLocaleString()}원</span>
        </div>
        <!-- 총 금액 -->
        <div class="flex justify-between items-center mb-4">
          <span class="text-lg font-bold text-gray-900">총 금액</span>
          <span class="text-xl font-bold text-blue-600">${(computed.selectedPrice || 0).toLocaleString()}원</span>
        </div>
        <!-- 액션 버튼들 -->
        <div class="space-y-2">
