export const CartItems = ({ productId, image, name, price, count, checked }) => `
    <div class="flex items-center py-3 border-b border-gray-100 cart-item" data-product-id="${productId}">
        ...
        <div class="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden mr-3 flex-shrink-0">
            <img src="${image}" alt="${name}" class="w-full h-full object-cover cursor-pointer cart-item-image" data-product-id="${productId}">
        </div>
        <!-- 상품 정보 -->
        <div class="flex-1 min-w-0">
            ...
            <input type="number" value="${count}" min="1" class="quantity-input w-12 h-7 text-center text-sm border-t border-b 
