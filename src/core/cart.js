...
export function toggleSelect(productId) {
  ...
  // 현재 체크박스는 브라우저가 이미 토글했으므로 footer 등만 업데이트 필요
  setTimeout(renderModalContent);
}
...
export function selectAll(selected) {
  ...
  persist();
  // 모든 행의 체크박스를 즉시 UI 에 반영하여 테스트와 동기화
  document.querySelectorAll(".cart-item-checkbox").forEach((el) => {
    el.checked = selected;
  });

  // footer 영역 업데이트는 한 프레임 뒤에 처리해도 무방
  setTimeout(renderModalContent);
}
...
function handleModalClick(e) {
  ...
  if (e.target.closest("#cart-modal-clear-cart-btn")) {
    // 클릭 액션 직후 DOM 이 재렌더링되면서 테스트 환경에서
    // element detached 오류가 발생할 수 있다.
    // 마이크로태스크 이후로 작업을 지연시켜 클릭 완료 → DOM 변경
    // 순서를 보장한다.
    setTimeout(() => {
      clearCart();
      updateCartBadge();
