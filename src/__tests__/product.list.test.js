...
describe('1. 상품 목록 로딩', () => {
  test('페이지 접속 시 로딩 상태가 표시되고, 데이터 로드 완료 후 상품 목록이 렌더링된다', async () => {
    ...
    expect(
      screen.getByText(
        /고양이 난간 안전망 복층 베란다 방묘창 방묘문 방충망 캣도어 일반형검정/i,
      ),
    ).toBeInTheDocument();
...
describe('2. 상품 목록 조회', () => {
  test('각 상품의 기본 정보(이미지, 상품명, 가격)가 카드 형태로 표시된다', async () => {
    ...
    // 상품명 확인
    expect(firstProduct.textContent).toMatch(
...
describe('4. 상품 정렬 기능', () => {
  test('상품을 가격순/인기순으로 정렬할 수 있다', async () => {
    ...
    expect(
      optionTexts.some(
        (text) => text.includes('낮은순') || text.includes('높은순'),
      ),
    ).toBe(true);
  });
  ...
  test('정렬 변경 시 목록에 반영된다', async () => {
    ...
    const expectProduct = (name, index = 0) => {
      ...
      expect(
        getByRole(product, 'heading', { level: 3, name }),
      ).toBeInTheDocument();
    };
    ...
    await waitFor(() => {
      expectProduct(
        '[1+1] 춘몽 섬유탈취제 섬유향수 룸스프레이 도플 패브릭 퍼퓸 217ml 블랑쉬',
      );
    });
...
describe('6. 상품 검색', () => {
  ...
  test('Enter 키로 검색이 수행할 수 있으며, 검색어와 일치하는 상품들만 목록에 표시된다', async () => {
    ...
    const productCards = [...document.querySelectorAll('.product-card')];
    expect(
      getByRole(productCards[0], 'heading', { level: 3, name: /젤리/i }),
    ).toBeInTheDocument();
    expect(
      getByRole(productCards[1], 'heading', { level: 3, name: /젤리/i }),
    ).toBeInTheDocument();
    expect(
      getByRole(productCards[2], 'heading', { level: 3, name: /젤리/i }),
    ).toBeInTheDocument();
  });
});
