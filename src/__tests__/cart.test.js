...
const addProductToCart = async (productName) => {
  ...
  const cartButton = productElement
...
describe.sequential('3. 장바구니 삭제', () => {
  ...
  test('삭제 버튼 클릭 시 해당 상품이 장바구니에서 제거된다', async () => {
    ...
    expect(
      document.querySelector('#cart-icon-btn span'),
    ).not.toBeInTheDocument();
  });
});
...
describe.sequential('5. 장바구니 전체 선택', () => {
  ...
  test('전체 해제 시 모든 상품의 체크박스가 해제된다', async () => {
    ...
    await userEvent.click(
      document.querySelector('#cart-modal-select-all-checkbox'),
    );
    expect(
      [...document.querySelectorAll('.cart-item-checkbox')].map(
        (v) => v.checked,
      ),
    ).toEqual([true, true]);
