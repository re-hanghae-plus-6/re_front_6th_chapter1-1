export const CartModal = ({ items = [], selectedItems = [] } = {}) => {
  ...
  const renderHeader = () => `
    <div class="sticky top-0 bg-white border-b border-gray-200 p-4 flex items-center justify-between">
      <h2 class="text-lg font-bold text-gray-900 flex items-center">
        ...
        장바구니
        ${totalItems > 0 ? `<span class="text-sm font-normal text-gray-600 ml-1">(${totalItems})</span>` : ""}
      </h2>
  ...
  const renderCartItem = (item) => `
    <div class="flex items-center py-3 border-b border-gray-100 cart-item" data-product-id="${item.id}">
      ...
      <label class="flex items-center mr-3">
        <input type="checkbox" ${selectedItems.some((selected) => selected.id === item.id) ? 'checked=""' : ""} class="cart-item-checkbox w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" data-product-id="${item.id}">
      </label>
      <!-- 상품 이미지 -->
  ...
  return `
    <div class="fixed inset-0 z-50 overflow-y-auto cart-modal-overlay modal-overlay" style="background-color: rgba(0, 0, 0, 0.5);">
      <div class="flex min-h-full items-end justify-center p-0 sm:items-center sm:p-4">
        <div class="relative bg-white rounded-t-lg sm:rounded-lg shadow-xl w-full max-w-md sm:max-w-lg max-h-[90vh] overflow-hidden cart-modal">
          ${renderHeader()}
          ${totalItems === 0 ? renderEmptyContent() : renderItemsContent()}
        </div>
      </div>
    </div>
  `;
